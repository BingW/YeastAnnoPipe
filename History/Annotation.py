...
class Feature():
    ...
    def judge_orf(self,strain_name):
        ...
        if count == 0:
            print "Warning: No stop codon @",self.product
            #print self.seq
        elif count > 2:
            print "Warning: mutiply stop codon @",self.product
            #print self.seq
        return True
...
def write_tbl(features,write_file):
    ...
    for feature in features:
        f.write(">Feature\t"+feature.identity+"\n")
        f.write("<"+str(feature.left)+"\t>"+str(feature.right)+"\tgene\n")
        f.write("\t\t\tgene\t"+feature.product+"\n")
                        ...
                        if contig_length > MIN_CONTIG:
                            ...
                            if contig.obj == "c9001" or contig.obj == "c9002" or\
                              contig.obj == "c9004" or contig.obj == "c9080":
                                contig.cmt = ">"+contig.component_id+" "+"[org=Saccharomyces kudriavzevii] \
                                [moltype=genomic] [strain="+strain_name+"][note=Contigs 9001, 9002, 9004, \
                                and 9080 are present only in Gal+ strains. They re syntenic to the GAL \
                                pseudogenes and surrounding regions. Illumina reads from all strains were \
                                allowed to assemble to either the functional or pseudogene alleles in these \
                                regions, whereas all other contigs had only IFO1802 as the reference strain. \
                                For each strain, only one allele is present and leads to significant coverage. \
                                See the full Methods in Hittinger et al. 2010 (PMID:20164837) for details.]"
